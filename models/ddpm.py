...
class Diffusion:
    ...
    def cal_fid_all(self, model, data_dir, device):
        ...
        labels_nosub = torch.zeros([100,], dtype=torch.long).to(device)
        labels_axion = torch.ones([100,], dtype=torch.long).to(device)
        labels_cdm = torch.full([100,], 2, dtype=torch.long).to(device)
